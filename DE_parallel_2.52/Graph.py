...
class Graph(object):
    ...
    def cal_weight_degree(self):
        ...
        for key in self.getVertices():
            ...
            if weight_degree>maxwd:
                maxwd = weight_degree
        self.maxwd = maxwd  # 该网络中的最大加权度
        return True
        ...
        if key not in self.verList.keys():
            ...
            for v in feats:
                if _X[int(v)] ==1:
                    ...
                    weight_number += (self.verList[key].getWeight(v)*self.weight_degrees[v]/self.maxwd)
...
def testGraph():
    path = 'D:/MyFile/AProgram/workspace/the-first-topic-new/data/processedData/cancer-feat_network_norm_pearson06.txt'
    g = Graph(path, 0.85)
